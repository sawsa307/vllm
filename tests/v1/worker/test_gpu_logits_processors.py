...
class RecordingProcessor(LogitsProcessor):
    ...
    def remove_request(self, req_idx: int) -> None:
        self.events.append(f"remove_request:{req_idx}")
...
def _make_sampler(processor: RecordingProcessor) -> Sampler:
    ...
    return Sampler(
        ...
        logitsprocs=[processor],
    )


def test_custom_processor_lifecycle_and_pipeline_position():
    """A custom processor is driven through the full lifecycle, runs after
    the built-in bias stage, and before temperature scaling."""
