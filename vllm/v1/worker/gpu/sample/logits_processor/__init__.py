...
__all__ = ["LogitsContext", "LogitsProcessor", "build_logitsprocs"]
...
def build_logitsprocs(
    ...
    """Load and instantiate custom logits processors for the V2 sampler.

    Args:
        vllm_config: passed through to each processor's constructor.
        req_states: the persistent batch state, passed through likewise.
        is_pooling_model: pooling models reject custom processors.
        custom_logitsprocs: processor classes or FQCN strings from
            ``ModelConfig.logits_processors``.

    Returns:
        Instantiated processors, entrypoint plugins first.
    ...
    return [ctor(vllm_config, req_states) for ctor in custom_logitsprocs_classes]
